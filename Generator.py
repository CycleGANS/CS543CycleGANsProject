...
def generator(input, D1, D2, U1, U2, DB1, DB2, UB1, UB2, res_dict):
...
D1 = tf.Variable(tf.truncated_normal([9,9,3,output_channels1] ,stddev=0.1))  
#Kernel size of 9x9 from 3 input channels (rgb). Number of output channels depends on how many you want in next layer.
DB1 = tf.Variable(tf.ones([output_channels1])/10)
D2 = tf.Variable(tf.truncated_normal([3,3,output_channels1,K], stddev=0.1))
...
DB2 = tf.Variable(tf.ones([K])/10)

# Define Variables for Residual Blocks
res_dict = {}
for i in range(1,NO_OF_RESIDUAL_BLOCKS+1):
    res_dict[i] = {'R1':tf.Variable(tf.truncated_normal([3,3,K,K] ,stddev=0.1)), 
                   'RB1':tf.Variable(tf.ones([J])/10),
                   'R2':tf.Variable(tf.truncated_normal([3,3,K,K] ,stddev=0.1)), 
                   'RB2':tf.Variable(tf.ones([J])/10)
                      }
     # Need to put the right input and output layer numbers


# Define Variables for upsampling
U1 = tf.Variable(tf.truncated_normal([3,3,number_of_input_channels,L], stddev=0.1))
UB1 = tf.Variable(tf.ones([L])/10)
U2 = tf.Variable(tf.truncated_normal([9,9,L,3] ,stddev=0.1))
#Here L is the number of output channels from the previous layer and is the number of input channels in this layer
UB2 = tf.Variable(tf.ones([3])/10)
